...
function moderationApiKey() {
  const runtime = env as unknown as ModerationEnvironment;
  return (runtime.IMAGE_MODERATION_API_KEY || runtime.OPENAI_API_KEY || "").trim();
}

export function moderationIsConfigured() {
...
export async function verifyProfilePhotoForSocial(file: File, dataUrl: string) {
