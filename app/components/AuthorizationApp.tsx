        ...
        {view === "dashboard" ? (
          ...
          <section className="inventory-workspace">
            ...
            <section className="inventory-section">
              ...
              {inventoryLoading ? <div className="empty-state"><span className="loader" /><p>Loading your inventory...</p></div> : vehicles.length === 0 ? <div className="empty-state inventory-empty"><div className="empty-icon">VIN</div><h3>Your first vehicle starts with a URL</h3><p>Paste one of the dealership's public VDP links above. The extracted record remains tied to its original source.</p></div> : <div className="vehicle-grid">{vehicles.map((vehicle) => <article className="vehicle-card is-clickable" key={vehicle.id} role="button" tabIndex={0} aria-label={`Create social post for ${[vehicle.year, vehicle.make, vehicleModelLabel(vehicle), vehicle.trim].filter(Boolean).join(" ")}`} onClick={() => startCreative(vehicle)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); startCreative(vehicle); } }}><div className="vehicle-image">{vehicle.imageUrls[0] ? <img src={vehicle.imageUrls[0]} alt={vehicle.title} loading="lazy" referrerPolicy="no-referrer" /> : <div className="vehicle-placeholder">No VDP image found</div>}<span className="source-badge">{vehicle.sourceHost}</span></div><div className="vehicle-body"><div className="vehicle-title"><div><span>{vehicle.year} {vehicle.make}</span><h3>{vehicleModelLabel(vehicle)}</h3><p>{vehicle.trim}</p></div>{vehicle.price && <strong>{formatPrice(vehicle.price, vehicle.currency)}</strong>}</div><dl><div><dt>VIN</dt><dd>{vehicle.vin || "Not detected"}</dd></div><div><dt>Stock</dt><dd>{vehicle.stockNumber || "Not detected"}</dd></div><div><dt>Photos</dt><dd>{vehicle.imageUrls.length}</dd></div></dl><div className="vehicle-source"><span>Captured {formatDate(vehicle.importedAt)}</span><a href={vehicle.sourceUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>View source ↗</a></div><button className="creative-next" onClick={(event) => { event.stopPropagation(); startCreative(vehicle); }}>{vehicle.imageUrls.length < 2 ? "Create social caption →" : "Create video storyboard →"}</button></div></article>)}</div>}
            </section>
          </section>
        ) : (
