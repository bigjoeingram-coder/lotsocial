            ...
            {!creativeVehicle ? <div className="empty-state"><h3>Select a vehicle first</h3><p>Return to My Inventory and choose a vehicle with at least two VDP photos.</p></div> : <form onSubmit={generateCreative}>
              ...
              <div className="creative-layout">
                <main className="creative-editor">
                  ...
                  <section className="creative-step"><div className="creative-step-title"><span>2</span><div><h2>Choose the video category</h2><p>Each category includes two rotating looks. The same vehicle keeps the same look on retries.</p></div></div><div className="creative-options"><label className={creativeStyle === "energetic" ? "selected" : ""}><input type="radio" name="style" value="energetic" checked={creativeStyle === "energetic"} onChange={() => { setCreativeStyle("energetic"); setCreativeDraft(null); }} /><strong>Fast cuts</strong><p>Surge + Velocity · rapid motion and high-energy music</p></label><label className={creativeStyle === "walkaround" ? "selected" : ""}><input type="radio" name="style" value="walkaround" checked={creativeStyle === "walkaround"} onChange={() => { setCreativeStyle("walkaround"); setCreativeDraft(null); }} /><strong>Walkaround</strong><p>Glide + Showcase · steady motion and balanced music</p></label><label className={creativeStyle === "premium" ? "selected" : ""}><input type="radio" name="style" value="premium" checked={creativeStyle === "premium"} onChange={() => { setCreativeStyle("premium"); setCreativeDraft(null); }} /><strong>Premium</strong><p>Noir + Gallery · cinematic motion and restrained music</p></label></div><div className="duration-options"><span>Target length</span>{[15,30,45].map((duration) => <button type="button" key={duration} className={creativeDuration === duration ? "active" : ""} onClick={() => { setCreativeDuration(duration); setCreativeDraft(null); }}>{duration}s</button>)}</div><fieldset className="gravy-options"><legend>Choose your Gravy</legend><p>Controls the embellishment—not the vehicle facts.</p><div>{([{ value: "none", label: "No Gravy", detail: "Straight facts" }, { value: "light", label: "Light Gravy", detail: "A little personality" }, { value: "extra", label: "Extra Gravy", detail: "Maximum safe sizzle" }] as const).map((option) => <label key={option.value} className={gravyLevel === option.value ? "selected" : ""}><input type="radio" name="gravy" value={option.value} checked={gravyLevel === option.value} onChange={() => { setGravyLevel(option.value); setCreativeDraft(null); setRenderJob(null); }} /><strong>{option.label}</strong><span>{option.detail}</span></label>)}</div></fieldset></section>
